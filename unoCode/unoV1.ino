/*
  ...
  주의: rawToEC(), rawToPH() 는 실측 캘리브레이션 계수입니다.
        - EC: 휴대용 수질측정기 기준 4점 선형회귀(739~1557uS/cm, R²≈0.998, offset≠0) + 온도 보정식
        - PH: 4.01 / 6.86 버퍼 2점 보정 (ph_ec_calibration_tool.ino)
        EC 온도 보정은 온도 센서가 없어 ASSUMED_TEMP_C(25.0) 고정값을 사용 — 사실상 no-op이며,
        실제 수온이 25도에서 크게 벗어나면 EC 값이 ±2%/°C 수준으로 오차가 날 수 있음(주석 참고).
        측정 구간(739~1557uS/cm)은 운용 목표(약 1634uS/cm)를 감싸므로 그 범위에서 신뢰도 높음.
        고농도 표준액(예: 12.88mS)이 추가되면 다점 재보정으로 범위 확장 권장.
*/
...
// ---------------------------------------------------------------
// 변환 함수 — 실측 캘리브레이션 계수
//   EC: 휴대용 측정기 기준 4점 선형회귀(739~1557uS/cm, R²≈0.998) + 25도 정규화
//   PH: 4.01/6.86 2점 보정 (ph_ec_calibration_tool.ino)
//   주의: 측정 구간(739~1557uS/cm)은 운용 목표(약 1634uS/cm)를 감싸므로 그 범위에서 정확.
//         고농도 표준액이 추가되면 다점 재보정으로 범위 확장 권장.
// ---------------------------------------------------------------
float rawToEC(int raw) {
  float voltage = raw * (5.0 / 1023.0);
  float ecAtTemp = 5378.748 * voltage + 142.854;  // uS/cm, 휴대용 측정기 기준 4점 회귀
